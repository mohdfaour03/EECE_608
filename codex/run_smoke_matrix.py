...
RESULTS_DIR = CODEX_DIR / "results" / "smoke_matrix"
SUMMARY_JSON = RESULTS_DIR / "smoke_matrix_summary.json"
SUMMARY_CSV = RESULTS_DIR / "smoke_matrix_summary.csv"
...
def make_train_config(
    *,
    ...
    return TrainExperimentConfig(
        experiment_name=f"codex_smoke_{dataset_name}",
        ...
        run=RunConfig(
            ...
            notes="Codex smoke matrix sidecar run.",
        ),
    )
...
def run_passive_attack(
    *,
    ...
    observation = run_passive_audit_once(
    ...
    return {
        ...
        "attack": attack_name,
        "status": "ok",
        "audit_regime": observation.audit_regime,
        ...
        "epsilon_lower_conservative": estimate.epsilon_lower_empirical,
        "epsilon_lower_point": estimate.epsilon_lower_empirical_point_estimate,
        "tightness_ratio_rdp": metrics.tightness_ratio,
        "privacy_loss_gap_rdp": metrics.privacy_loss_gap,
    ...
    *,
    ...
    if dataset_name not in {"mnist", "cifar10"}:
        return {
            ...
            "reason": "canary smoke runner currently supports image datasets only",
        }
    ...
    canary_train_config = TrainExperimentConfig(
        experiment_name=f"{config.experiment_name}_{attack_name}",
        ...
        run=RunConfig(
            ...
            notes="Codex canary smoke retraining run.",
        ),
    )
    ...
    outcome = train_dp_sgd(
        ...
        save_checkpoint=False,
        run_descriptor=f"{attack_name}_{dataset_name}",
    ...
    metrics = compute_privacy_tightness_metrics(
        epsilon_upper_theory=outcome.record.epsilon_upper_theory,
        epsilon_lower_empirical=estimate.epsilon_lower_empirical,
    )
    return {
        "dataset": dataset_name,
        ...
        "epsilon_upper_rdp": outcome.record.epsilon_upper_theory,
        "epsilon_upper_tighter": outcome.record.epsilon_upper_pld,
        "tighter_upper_backend": outcome.record.pld_accounting_backend,
        "epsilon_lower_conservative": estimate.epsilon_lower_empirical,
        "epsilon_lower_point": estimate.epsilon_lower_empirical_point_estimate,
        "tightness_ratio_rdp": metrics.tightness_ratio,
        "privacy_loss_gap_rdp": metrics.privacy_loss_gap,
        ...
        "num_member_samples": estimate.num_member_samples,
        "num_nonmember_samples": estimate.num_nonmember_samples,
        "canary_inserted_example_count": insertion_result.inserted_example_count,
    }
    ...
    for spec in dataset_specs:
        ...
        try:
            ...
            for attack_name, attack_fn in [
                ("passive_logit_margin", lambda: run_passive_attack(
                    dataset_name=dataset_name,
                    attack_name="passive_logit_margin",
                    score_type="logit_margin",
                    training_case=training_case,
                    audit_seed=401,
                )),
                ("passive_score_fusion", lambda: run_passive_attack(
                    dataset_name=dataset_name,
                    attack_name="passive_score_fusion",
                    score_type="score_fusion",
                    training_case=training_case,
                    audit_seed=402,
                )),
                ("canary_random", lambda: run_canary_attack(
                    dataset_name=dataset_name,
                ...
                try:
                    ...
                    summary.append(row)
                    logger.info(
                        "Completed %s on %s status=%s eps_lower=%s",
                        attack_name,
                        dataset_name,
                        row.get("status"),
                    ...
                    logger.exception("Attack %s failed on %s", attack_name, dataset_name)
        except Exception as exc:  # pragma: no cover - smoke harness
            summary.append(
                {
                    "dataset": dataset_name,
